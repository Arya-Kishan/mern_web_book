...
const AdminError = () => {
  const { data, isLoading } = useGetAllErrorQuery();
        ...
        {data ? (
          data.length < 1 ? (
            ...
            data.map((e) => (
              <div
                key={e._id}
                className="w-[90%] min-h-[300px] h-fit flex flex-col justify-between bg-bg-card p-2 text-white overflow-scroll"
              >
                <div>
                  <p className="text-[16px] text-customGreen font-bold">
                    Error :{" "}
                  </p>
                  <p>{e.error}</p>
                </div>
