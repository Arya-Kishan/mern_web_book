...
const SavedMessageCard = ({ e, messageBubblePress }) => {
  ...
  return (
    ...
    >
      <div
        className={`min-w-[25%] max-w-[80%] h-fit ${e.sender._id == loggedInUser._id ? "bg-blue-950" : "bg-blue-600"} flex flex-col p-2 rounded-xl`}
        onClick={() => messageBubblePress(e, "self")}
      >
        ...
        <div className="w-full h-[10px] flex justify-end text-[10px] items-center gap-1">
          ...
          {showTick && (
            <MyImage src={sentIcon} className={"w-[10px] h-[10px]"} />
          )}
        </div>
      </div>
    </div>
