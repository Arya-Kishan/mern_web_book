...
export const loginUser = async (req, res) => {
  try {
    ...
    res
      ...
      .json({ message: "LOGGING USER", data: user });
  } catch (error) {
    console.log(error);
...
export const searchUsers = async (req, res) => {
  try {
    ...
    res
      .status(400)
      .json({ message: "ERROR IN SEARCHING USERS", Error: error, data: null });
  }
};
...
export const addFriend = async (req, res) => {
  try {
    ...
    if (user.friends.includes(receiverRequestUserId)) {
      ...
      let newFriends = await User.findByIdAndUpdate(receiverRequestUserId, {
        $push: { friends: senderRequestUserId },
      });
      let newFriends2 = await User.findByIdAndUpdate(senderRequestUserId, {
        $push: { friends: receiverRequestUserId },
      });
    ...
    res
      .status(400)
      .json({ message: "ERROR IN ADDING FRIEND", Error: error, data: null });
  }
};

export const sendOnlineNotiToAdmin = async ({ category, message, data }) => {
  try {
    //   console.log("data : ", data);
    const users = await User.find({
      $or: [{ _id: { $in: data.activeUsers } }, { role: "admin" }],
    })
      .select("_id email role name FCMtoken")
      .lean();
    const adminUser = users.find((user) => user.role == "admin");
    const messageData = users
      .map((user) => `${user.name} - ${user.email} is online `)
      .join("\n");
    const result = await sendNotificationFCM(
      adminUser.FCMtoken.deviceToken,
      category,
      messageData,
    );
    return null;
